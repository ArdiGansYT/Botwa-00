...
/******** OWNER NUMBER**********/
const ownerNumber = ["6285282996146@s.whatsapp.net","6283102650464@s.whatsapp.net"] // nomer 08528299...Jangan diubah!! ntar error
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','pink'),color('∆','pink'),color(']','pink'),color('Sqan Tu Ngab Work Ya... Subscribe','pink'),color('YOU','pink'),color('TUBE','pink'),color('ArdiGansYT','pink'))
})
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (args.length < 1) return client.sendMessage(from, 'Diperlukan kode bahasa!!', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('lah teks nya kepanjangan 🐗')
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *PatrickBot BROADCAST*❯\n\n${body.slice(4)}`})
						}
